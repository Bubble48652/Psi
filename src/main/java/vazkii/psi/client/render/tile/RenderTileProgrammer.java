...
public class RenderTileProgrammer implements BlockEntityRenderer<TileProgrammer> {
	...
	public void render(TileProgrammer te, float partialticks, PoseStack ms, MultiBufferSource buffers, int worldLight, int overlay) {
		if (te.isEnabled()) {
			...
			mc.font.drawInBatch(I18n.get("psimisc.name"), 20, SpellGrid.GRID_SIZE * 18, color, false, ms.last().pose(), buffers, false, 0, 0xF000F0);
			mc.font.drawInBatch(te.spell.name, 58, SpellGrid.GRID_SIZE * 18, color, false, ms.last().pose(), buffers, false, 0, 0xF000F0);
